...
public class SimpleBankingApp {
	...
	public static void printAllUsers() {
		...
		for (int i = 0; i < users.size(); i++) {
			...
			String formattedUser = String.format("%-25s| %-15s| %-15s| %-15s| %-15s",
					user.getUsername(), user.getPassword(), user.getFirstName(),
					user.getLastName(), user.getMobileNumber());
	...
	public static void printAllAccounts() {
		...
		for (int i = 0; i < accounts.size(); i++) {
			...
			String formattedDate = "(" + dateFormat.format(account.getAccountOpeningDate()) + ")";
			String formattedAccount = String.format("%-10s| %-30s| %-10s| %-15s| $%-15s",
					account.getAccountNumber(), account.getUsernameOfAccountHolder(), account.getAccountType(),
					formattedDate, getBalance(account.getAccountNumber()));
			System.out.println(formattedAccount);
		}
